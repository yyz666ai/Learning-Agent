...
- 已有课程、用户笔记和历史对话保留原文；不会提供旧课件的“翻译”副本。切换语言后创建或重新生成的 Plan、课件、练习和答疑才使用所选语言，原课程不会被覆盖。
- 语言偏好保存在 `userdir/u_<id>/preferences.json`。保存语言偏好失败会显示重试提示；界面语言 `locale` 与 Python / Go 等编程语言 `language` 是两个独立字段。
