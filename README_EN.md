...
Existing courses, conversations and learner notes are not rewritten. There is no translation view for old lessons. After changing language, newly created or regenerated Plans, lessons, exercises and chat replies use the selected language; existing material remains unchanged.

Preferences live in `userdir/u_<id>/preferences.json`. Saving errors offer retry instead of claiming success. Interface `locale` is separate from the programming `language` field.
