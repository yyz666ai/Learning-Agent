...
1. 意图 Agent 先确认目标岗位、基础和技术栈。
2. 询问是否有从小红书、面经或 JD 收集的真实题；用户直接在同一个输入框粘贴。
3. 系统保留原文，拆分、去重、分类并加入个人面试题库；入库完成前不生成 Plan。
4. Plan Agent 读取完整岗位、技术栈和已收录题目，按能力域重排并补关联追问。
5. 用户选择逐题从头讲、系统学习或先测后学。
6. 没有答案的题生成参考答案、rubric、常见遗漏和追问。
7. PPT 先显示题目，用户思考或口述后再展开答案。
8. 用户按没想起来、困难、顺利记录掌握度。
9. 后续新增题目只调整待学内容，不倒退已获得的整体进度。
10. 最后进行两道实战题和一轮模拟面试。
...
4. 后端验证题目、选项、答案和重复情况。
5. 有活动讲义时，把新题插入当前 HTML PPT 的总结页之前，并同步注册题库；没有活动讲义时只写题库。
6. 前端重新读取讲义，定位到第一道追加题并提示新增数量。
7. 用户可以立即在 PPT 点击作答，也可以留到题库复习。

## 15. 用户记忆与项目切换工作流

1. 每轮 onboarding 判断都原子覆盖 `intent-state.json`，同时向 `intent-events.jsonl` 追加证据和槽位变化。
2. Plan 确认时生成面向机器的 `profile.json` 和面向用户的 `profile.md`。
3. 成功的流式对话按角色写入 `conversation-events.jsonl`；浏览器记录只用于快速恢复界面。
4. 新建或切换学习项目时，整体快照画像、Plan、课程、题库、复习、面试题和真实练习目录。
5. 恢复失败时回滚旧快照，不能只恢复前端画面而留下错误的后端课程状态。
