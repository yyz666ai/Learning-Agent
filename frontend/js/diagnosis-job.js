...
(function expose(root, factory) {
  ...
  const stale = () => Object.assign(new Error("这轮诊断已不属于当前页面。"), {name: "AbortError"});
  ...
  async function readJob(url, options = {}, fetcher = fetch, timeoutMs = 8000) {
    ...
    try {
      ...
      if (!response.ok) {
        const error = new Error(value.detail?.message || "诊断状态暂时不可用，请重试。");
  ...
  function statusText(job) {
    if (job.status === "unknown") return "暂时无法读取状态，正在重连。尚不能确认后台是否完成。";
    const phase = {queued: "已排队，等待生成诊断题", generating: "正在生成与你的目标相关的诊断题", validating: "正在校验题目与选项", repairing: "正在修正题目结构", completed: "诊断题已准备好", cancelled: "这轮诊断已取消", failed: "诊断暂时未完成", interrupted: "服务重启，原任务已中断"};
    const elapsed = Number.isFinite(job.elapsed_seconds) ? ` · 已等待 ${Math.floor(job.elapsed_seconds)} 秒` : "";
    return (phase[job.phase] || phase[job.status] || "正在读取诊断任务状态") + elapsed;
  }
  ...
  async function waitForDiagnosis({payload, fetcher = fetch, onStatus = () => {}, isCurrent = () => true,
    ...
    throw new Error("暂时无法确认诊断结果。你的请求标识已保留，点击重试将读取同一任务，不会重复生成。");
  }
